...
import { EditorUi, MXGeometry, MXCell, MxWindow, ClassItem } from '../utils/types.js';
...
interface WhileNodeConstructorWindowInstance {
    window: MxWindow & { contentWrapper?: HTMLElement };
}

// Окно конструктора узлов while
export const WhileNodeConstructorWindow = function (
    this: WhileNodeConstructorWindowInstance,
    editorUi: EditorUi,
    x: number,
    y: number,
    w: number,
    h: number
): WhileNodeConstructorWindowInstance {
    const self = this;
    ...
    const win = new mxWindow(
        getTextByLocale("TitleWhileNodeConstructorWindow"),
        div,
        x,
        y,
        w,
