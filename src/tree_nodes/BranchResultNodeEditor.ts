...
interface BranchResultNodeEditorWindowInstance {
    window: MxWindow & { contentWrapper?: HTMLElement };
}

// Окно редактирования узлов условий (branch result)
export const BranchResultNodeEditorWindow = function (
    this: BranchResultNodeEditorWindowInstance,
    cell: ExtendedMXCell,
    editorUi: EditorUi,
    x: number,
    y: number,
    w: number,
    h: number
): BranchResultNodeEditorWindowInstance {
    const self = this;
    ...
    const win = new mxWindow(
        getTextByLocale("TitleBranchResultNodeEditorWindow"),
        div,
        x,
        y,
        w,
