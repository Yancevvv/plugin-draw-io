...
import { EditorUi, MXGeometry, MXCell, MxWindow } from '../utils/types.js';
...
interface SwitchCaseNodeConstructorWindowInstance {
    window: MxWindow & { contentWrapper?: HTMLElement };
}

// Окно конструктора узлов switch-case
export const SwitchCaseNodeConstructorWindow = function (
    this: SwitchCaseNodeConstructorWindowInstance,
    editorUi: EditorUi,
    x: number,
    y: number,
    w: number,
    h: number
): SwitchCaseNodeConstructorWindowInstance {
    const self = this;
    ...
    const win = new mxWindow(
        getTextByLocale("TitleSwitchCaseNodeConstructorWindow"),
        div,
        x,
        y,
        w,
