...
interface BranchResultNodeConstructorWindowInstance {
    window: MxWindow & { contentWrapper?: HTMLElement };
}

// Окно конструктора узлов условий
export const BranchResultNodeConstructorWindow = function (
    this: BranchResultNodeConstructorWindowInstance,
    editorUi: EditorUi,
    result: boolean | null,
    x: number,
    y: number,
    w: number,
    h: number
): BranchResultNodeConstructorWindowInstance {
    const self = this;
    ...
    let btnCreateNodeInText = mxUtils.button(getTextByLocale("Create"), function (): void {
        ...
        if (theGraph.isEnabled() && !theGraph.isCellLocked(theGraph.getDefaultParent())) {
            ...
            if (result === null) {
                geometry = { x: pos.x, y: pos.y, width: 66, height: 30 };
                style = "rounded=1;whiteSpace=wrap;html=1;fillColor=#e6e6e6;strokeColor=#666666;editable=0;";
            } else if (result) {
                geometry = { x: pos.x, y: pos.y, width: 66, height: 30 };
                style = "rounded=1;whiteSpace=wrap;html=1;fillColor=#d5e8d4;strokeColor=#82b366;editable=0;";
            } else {
                geometry = { x: pos.x, y: pos.y, width: 66, height: 30 };
                style = "rounded=1;whiteSpace=wrap;html=1;fillColor=#f8cecc;strokeColor=#b85450;editable=0;";
            }
    ...
    const win = new mxWindow(
        getTextByLocale("TitleBranchResultNodeConstructorWindow"),
        div,
        x,
        y,
        w,
