...
interface ActionNodeEditorWindowInstance {
    window: MxWindow & { contentWrapper?: HTMLElement };
}

// Окно редактирования узлов действия
export const ActionNodeEditorWindow = function (
    this: ActionNodeEditorWindowInstance,
    cell: ExtendedMXCell,
    editorUi: EditorUi,
    x: number,
    y: number,
    w: number,
    h: number
): ActionNodeEditorWindowInstance {
    const self = this;
    ...
    const type = cell.value.getAttribute('typeVar');
    if (type) {
    ...
    nameVarInBlockly.value = cell.value.getAttribute('nameVar') || '';
    ...
    if (type) {
        for (let index = 0; index < selectClassInBlockly.options.length; ++index) {
            if (selectClassInBlockly.options[index].value === type) {
                selectClassInBlockly.options[index].selected = true;
                break;
            }
        }
    }
    ...
    const win = new mxWindow(
        getTextByLocale("TitleActionNodeEditorWindow"),
        div,
        x,
        y,
        w,
